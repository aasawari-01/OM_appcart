...
class LostFoundService {
  ...
  Future<Map<String, dynamic>> updateLostFound(int lostFoundID, Map<String, dynamic> data, List<File> files) async {
    ...
    print("DEBUG: Update URL: ${Uri.parse('${AppUrls.baseUrl}${AppUrls.createLostFound}').replace(queryParameters: {'lostFoundID': lostFoundID.toString()})}");
    print("DEBUG: Payload Data: ${request.fields['lostFoundData']}");

    final streamedResponse = await request.send().timeout(const Duration(seconds: 30));
    final response = await http.Response.fromStream(streamedResponse);
    final Map<String, dynamic> jsonBody = jsonDecode(response.body);
    print("jsondata==$jsonBody");
    if (response.statusCode == 200) {
      return jsonBody;
    } else {
      print("msgg===${jsonBody['message']}");
